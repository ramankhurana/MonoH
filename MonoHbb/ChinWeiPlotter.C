...
void plot_Asymptotic_altascombine(string outputdir, string mode)
{
...
//  gROOT->LoadMacro("CMS_lumi.C");
  ...
  for(int n=0;n<nXm;n++)
  {
    ...
    sprintf(limitfilename,"higgsCombineTest.Asymptotic.mH%d.root",Xmass[n]);
...
//    leg->AddEntry(grthSM10, "#sigma_{TH} x BR(Z' #rightarrow " + VV + "), #tilde{k}=0.20", "L"); // #rightarrow 2l2q
  ...
  postname = "MonoHToDMbb_Asymptotic_Log_"+mode+".pdf";
  name = outputdir+postname;
  cMCMC->SaveAs(name.c_str());
