...
#include "TStyle.h" 
 ...
 TString DirPreName = "AnalysisHistograms_MergedSkimmedV12_Puppi_V1/zj";
 dirpathname = "05122016"; //.Form("%d%1.2d%d",ltm->tm_mday,1 + ltm->tm_mon,1900 + ltm->tm_year);
 ...
 ofstream mout;
 mout.open(DirPreName+dirpathname +"/Signal"+dirpathname +"Integral.txt",std::ios::app);
 ofstream rout;
 rout.open(DirPreName+dirpathname +"/Signal"+dirpathname +"Integral.html",std::ios::app);
 ofstream tableout;
 tableout.open(DirPreName+dirpathname +"/Signal"+dirpathname +"IntegralWithError.txt",std::ios::app);                                                                  
 TString outputshapefilename = DirPreName+dirpathname +"/Signal.root";
...
ofstream metbinsout_3;
 ...
 metbinsout_1.open(DirPreName+"METBIN_1/Signal"+dirpathname +"Integral.txt",std::ios::app);
 metbinsout_2.open(DirPreName+"METBIN_2/Signal"+dirpathname +"Integral.txt",std::ios::app);
 metbinsout_3.open(DirPreName+"METBIN_3/Signal"+dirpathname +"Integral.txt",std::ios::app);
}
...
TString filenamepath("AnalysisHistograms_MergedSkimmedV12_Puppi_V1/zj/"); 
// DYJets 0
...
TString histnameString("h_met_0");
...
if("Signal" =="histfacFatJet_ZLight"){
histolabel = "Zj CR(boosted)";}
if("Signal" =="histfacFatJet_WHeavy" ){
...
bool nminus = 0;
TLatex *tt;
if(("Signal" == "ElectronNMinus1E") || ("Signal" == "ElectronNMinus1B") ){
nminus =1;
...
TH1F* h_prefit;
TFile* fprefit;
TString prefitfilename = "PREFITDIR/Signal.root";
if(0){
...
if("h_met_0"=="h_cutFlow0"){
   if("Signal" == "MonoHFatJetSelection_JetAndLeptonVeto"){
...
//  h_err->SetLineColor(kGray+3);
  ...
  if(!0)   hs->SetMaximum(maxi *1.8);
  if(0)    hs->SetMaximum(maxi *500);
  //if(!0) hs->SetMaximum(0.4);
  }else{
  if(0)    hs->SetMinimum(0.1);
  if(!0)   hs->SetMinimum(1);
  if(!0)   hs->SetMaximum(maxi *1.70);
...
hs->GetXaxis()->SetNdivisions(508);                                                                                                                                             
  ...
  hs->GetYaxis()->SetTitle("Events");                                                                                                                                                 if(!0){   hs->GetYaxis()->SetTitle("Events/"+binwidth_);                                   }
  ...
  hs->GetXaxis()->SetRangeUser(200,1000);  
  hs->GetXaxis()->SetNdivisions(508); 
...
if(0){ c1_1->SetLogx(0);                                                                                                           
 ...
 DataMC->GetXaxis()->SetRangeUser(200,1000);
...
DataMC->Draw("P e1 same");
 ...
 TLine* line0= new TLine(200,1,1000,1);
...
//=======================================================================
 ...
 Int_t binxmax = xaxis->FindBin(1000);
...
float zh_error = TMath::Sqrt(pow(Integral_Error[6],2));
  ...
  mout << "Signal_h_met_0"            <<  " a b"<<std::endl; 
...
tableout<< " "<<std::endl;
 ...
 c12->Draw();
if(!0){
 c12->SaveAs(DirPreName+dirpathname +"/MonoHPdf/zj_h_met_0.pdf");
 c12->SaveAs(DirPreName+dirpathname +"/MonoHPng/zj_h_met_0.png");
 c12->SaveAs(DirPreName+dirpathname +"/MonoHROOT/zj_h_met_0.root");                                                                         
 rout<<"<hr/>"<<std::endl;
 rout<<"<table class=\"\"> <tr><td><img src=\""<<"DYPng/Signal_h_met_0.png\" height=\"400\" width=\"400\"></td>   </tr> </table>"<<std::endl;

}
 
if(0){
 c12->SaveAs(DirPreName+dirpathname +"/MonoHPdf/Signal_h_met_0_log.pdf");
 c12->SaveAs(DirPreName+dirpathname +"/MonoHPng/Signal_h_met_0_log.png");
 c12->SaveAs(DirPreName+dirpathname +"/MonoHROOT/Signal_h_met_0_log.root");                                                                        
}
